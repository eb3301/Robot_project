...
class DetectionManager(Node):
    ...
    #     self.get_logger().info(f"omega: {self.angular_velocity}")

    #     # Soglia minima per considerare rotazione (per evitare falsi positivi)
    #     rot_threshold = 0.05

    #     if self.angular_velocity > rot_threshold: #aggiungi vincolo su vel lin <vmax
    ...
    def encoder_callback(self, msg: Encoders):
        ...
        if abs(w) > rot_threshold or abs(v) > vel_threshold: #aggiungi vincolo su vel lin <vmax
            if self.detection_active:
                self.get_logger().info("Robot in rotazione: fermo la detection.")
                self.call_set_bool(self.stop_client, True)
                self.detection_active = False
