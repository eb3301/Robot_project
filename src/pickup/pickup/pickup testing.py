...
class PickupPublisher(Node):
    ...
    def __init__(self):
        super().__init__('move_servos_series_publisher')
...
def main(args=None):
    print('Hi from pickup.')
    rclpy.init(args=args)

    pickup_publisher = PickupPublisher()

    rclpy.spin(pickup_publisher)

    # Destroy the node explicitly
    # (optional - otherwise it will be done automatically
    # when the garbage collector destroys the node object)
    pickup_publisher.destroy_node()
    rclpy.shutdown()
